...
int calc_multiplicity(const Array<rVector3d> &cluster,
  ...
  //cout << equiv << endl; This may has error.
  return equiv.get_size();// The size of symmetrically equivalent clusters
}
...
// New added
void write_full_clusters_symops(ofstream& fullout, const MultiCluster &cluster, const rMatrix3d &cell, const Array<rMatrix3d> &point_op, const Array<rVector3d> &trans) {
  rMatrix3d inv_cell=!cell;
  fullout << "Representative cluster: " << endl << cluster.clus <<endl;
  for (int op=0; op<point_op.get_size(); op++) {
    MultiCluster *ptransfo_cluster=new MultiCluster(cluster.clus.get_size());
    apply_symmetry(ptransfo_cluster,point_op(op),trans(op),cluster);
    fullout << "Symmetry operation:" << op  << endl;
    fullout << "Point:" <<endl;
    fullout << point_op(op) <<endl;
    fullout << "Translation:" <<endl;
    fullout << trans(op) <<endl<<endl;
    fullout << "Transformed cluster:"<< endl;
    fullout << ptransfo_cluster->clus <<endl;
    delete ptransfo_cluster;
  }
}
//
...
// Still new added
void write_reduced_clusters(ofstream& reducedout, const MultiCluster &cluster, const rMatrix3d &cell, const Array<rMatrix3d> &point_op, const Array<rVector3d> &trans) {
  rMatrix3d inv_cell=!cell;
  LinkedList<MultiCluster> list;
  reducedout << "Representative cluster: " << endl << cluster.clus <<endl;
  for (int op=0; op<point_op.get_size(); op++) {
    MultiCluster *ptransfo_cluster=new MultiCluster(cluster.clus.get_size());
    apply_symmetry(ptransfo_cluster,point_op(op),trans(op),cluster);
    ...
    if (!i) {
      list << ptransfo_cluster;
      reducedout << "Symmetry operation:" << op  << endl;
      reducedout << "Point:" <<endl;
      reducedout << point_op(op) <<endl;
      reducedout << "Translation:" <<endl;
      reducedout << trans(op) <<endl<<endl;
      reducedout << "Transformed cluster:"<< endl;
      reducedout << ptransfo_cluster->clus <<endl;
    }
    else {
      delete ptransfo_cluster;
    }
...
void find_equivalent_clusters(Array<MultiCluster> *pclusters, const MultiCluster &cluster,
  ...
  rMatrix3d inv_cell=!cell;
  LinkedList<MultiCluster> list;
  //cout << "Representative cluster: " << endl << cluster.clus <<endl;
  //clusters_symops << "Representative cluster: " << endl << cluster.clus <<endl;
  for (int op=0; op<point_op.get_size(); op++) {
    MultiCluster *ptransfo_cluster=new MultiCluster(cluster.clus.get_size());
    apply_symmetry(ptransfo_cluster,point_op(op),trans(op),cluster);
    //cout << "Symmetry operation:" << op  << endl;
    //cout << "Point:" <<endl;
    //cout << point_op(op) <<endl;
    //cout << "Translation:" <<endl;
    //cout << trans(op) <<endl<<endl;
    //cout << "Transformed cluster:"<< endl;
    //cout<< ptransfo_cluster->clus <<endl;
    LinkedListIterator<MultiCluster> i(list);
    for ( ; i; i++) {
      if (equivalent_mod_cell(*i,*ptransfo_cluster,inv_cell)) break;
    }
    if (!i) {
      list << ptransfo_cluster;// Only 'not equivalent_mod_cell' or 'new' cluster countered.
      //cout << "Symmetry operation:" << op  << endl;
      //cout << "Point:" <<endl;
      //cout << point_op(op) <<endl;
      //cout << "Translation:" <<endl;
